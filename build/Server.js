...
server.resource("get-server-Boards", "boards://all", {
...
server.resource("get-board-by-id", new ResourceTemplate("board://{boardId}/", { list: undefined }), {
...
server.tool("create-board-in-DB", "create new board", {
    boardTitle: z.string(),
}, {
...
server.tool("create-task-list-in-DB", "create new List in database", {
...
server.tool("create-task-in-DB", "create new task in database", {
    parentBoardId: z.string(),
    ...
    try {
        const getResponse = await api.get(`/board/${params.parentBoardId}`);
        const list = getResponse.data.lists.find((list) => list._id === params.parentListId);
        let taskIndex = 0;
        if (!list.taskList) {
